...
| [Render-Speed-Analysis-and-Plan.md](./Render-Speed-Analysis-and-Plan.md) | **Speed-focused:** why HyperFrames renders faster than the video is long (seek-don't-play, beginFrame, JPEG flipbooks, parallelism), where Studio Pro's MediaBunny export spends its time today (with code refs), and the phased plan to take exports from real-time to faster-than-real-time. |
| [Design-Templates-and-Skills-Plan.md](./Design-Templates-and-Skills-Plan.md) | **Design-focused:** how HyperFrames packages design (frame.md / design.md inversion, named designs, catalog blocks) and production (the 19 skills / creation workflows), mapped onto Studio Pro's existing preset layers (`DEFAULT_PRESETS`, `subtitleConfig`, `markdownConfig`), plus the phased plan for **Project Design Templates = collections of customizable typed presets** (`.sptpl`). |
| [Agent-Authoring-and-Automation-Plan.md](./Agent-Authoring-and-Automation-Plan.md) | **Consolidated automation plan:** the four-way analysis (Studio Pro code audit + the "Remotion Full Tutorial" video + Remotion + HyperFrames), a pros/cons comparison table, and the three-pillar plan — **Templates** (`.sptpl`), **Skills** (Markdown ↔ `.spcomp` agent loop + AGENTS.md), and the **Automation substrate** (determinism + FTRT export + AI panel) — with ranked features and milestones M0–M6. |
| [Projects-as-Templates-and-Md-Props-Plan.md](./Projects-as-Templates-and-Md-Props-Plan.md) | **Design-by-example:** the three user ideas analyzed as one system — saved projects **are** templates (a "Save as Template" derivation strips content, keeps style), a **workspace folder** (`projects/` + `templates/` + `skills/` via the File System Access API) auto-generates **frame.md-style skills from real projects**, and **Markdown props** (front-matter + `[key:value]` tags) turn the script into a declarative video spec. |
| [Markdown-Props-Spec.md](./Markdown-Props-Spec.md) | **The P1 implementation spec:** exact front-matter key set (26 keys → `markdownConfig`/`subtitleConfig`/globals), the `[key:value]` element tag list, the precedence chain (element > template > front-matter > defaults), the two-way UI↔script sync, and the backward-compatibility guarantees + acceptance criteria. |
