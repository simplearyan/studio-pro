...
| **C. Readability** ✅ done (2026-08-17) | Typography floors, `tabular-nums`, Compare row restructure, note text update | Verified live: zero `text-[10px]` left in the modal (all bumped to 11 px; dragTooltip excluded — it's a fixed global tooltip); `tabular-nums` on every mono value (Compare results, probe status, last-run, timer/ETA/progress, fps detail, custom-fps + probe + start/end inputs, success-card values); Compare rows now `items-baseline` + `gap-3`; stale M1b wording replaced. `whitespace-nowrap`/stacking deferred to Phase D (responsive) |
